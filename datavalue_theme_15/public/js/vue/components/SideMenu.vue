<template>
  <div class="side-menu" id="side-menu">
    <div class="side-mobile-menu animated" :class="(isRTL)?'slideInRight':'slideInLeft'">
      <button type="button" class="mobile-back-modules" @click="mobile_back_modules()" v-if="is_shown_mobile_menu">
        <i class="fal" :class="(isRTL)?'fa-arrow-right':'fa-arrow-left'"></i> {{ frappe._(active_module.name) }}
      </button>
      <ul class="list-unstyled mobile-modules-list" v-if="!is_shown_mobile_menu">
        <li v-for="(module, index) in modules_list" :style="'animation-delay:'+(0.06*(index+1))+'s;'" :class="(active_module.name==module.name)?'active':''">
          <a @click="open_module(module.name,true)">
      ...
      <ul class="list-unstyled mobile-modules-menu-list" v-if="is_shown_mobile_menu && module_items_list[active_module.name] && module_items_list[active_module.name].length">
        ...
        <!--          <a @click="open_dashboard(active_module.name)" :data-dashboard="active_module.name">{{ frappe._("Dashboard") }}</a>-->
        <!--        </li>-->
      ...
      <div class="modules-menu-list">
        <div class="dv-row dv-row-sm animated fadeInLeft">
          <div class="dv-col-6 animated faster fadeInLeft" v-for="(module, index) in modules_list" :style="'animation-delay:'+(0.06*(index+1))+'s;'">
            <a @click="open_module(module.name)" class="animated-tada" :class="(active_module.name==module.name)?'active':''">
              <i :class="get_module_icon(module.name,module.icon)+' animated-icon'"></i>
              {{ (module.label) ? module.label : module.name }}
            </a>
          </div>
    ...
    <div class="side-menu-icons" :class="(theme_settings.show_icon_label && theme_settings.show_icon_label=='1')?'menu-icons-with-label':''">
      <ul class="list-unstyled">
        <li v-for="module in modules_list" :class="(active_module.name==module.name)?'active':''">
          <a @click="open_module(module.name)" :title="(module.label)?module.label:module.name" :data-toggle="(theme_settings.hide_icon_tooltip && theme_settings.hide_icon_tooltip=='1')?'':'tipsy'" class="animated-tada">
...
let theme_settings = ref({});
let current_doctype = ref('');
let current_page = ref('');
let is_dashboard = ref(false);
let is_shown_mobile_menu = ref(false);
let menu_items_animate = ref('slideInLeft');
let module_icon = ref({
...
let module_items_list = ref({});
let active_module = ref({
  name: 'Home'
});
...
const get_module_name_from_doctype = () => {
  ...
  current_doctype = (route && route.length >= 2 && route[1]) ? route[1] : '';
  if (route && route[0] && route[0] == 'dashboard-view') {
    current_doctype = 'Dashboard';
  }
  is_dashboard = (route && route.length == 2 && route[1] && route[0] && route[0] == 'dashboard-view') ? true : false;
  frappe.call({
    type: 'POST',
    method: 'datavalue_theme_15.api.get_module_name_from_doctype',
    args: {
      doc_name: current_doctype,
      current_module: (active_module && active_module.name) ? active_module.name : ''
    },
    callback: function (response) {
      ...
      // }
      is_shown_mobile_menu = is_mobile();
      module_menu_list(current_module);
    }
  });
};

const module_menu_list = (current_module, is_mobile = false) => {
  console.log('current_module=', current_module);
  const route = frappe.get_route();
  ...
  if (modules_list && modules_list.length) {
    active_module = modules_list.filter(function (module) {
      return module.name == _module;
    })[0];
    if (active_module && active_module.name) {
      if (module_items_list[active_module.name] && module_items_list[active_module.name].length) {
        after_side_menu_items();
      } else {
        get_module_items(active_module.name, (items) => {
          if (items.message && items.message.cards && items.message.cards.items) {
            setTimeout(() => module_items_list[active_module.name] = items.message.cards.items, 10);
            setTimeout(() => console.log('=========module_items_list_[' + active_module.name + ']__', module_items_list), 10);
          }
          after_side_menu_items();
        });
      }
    ...
    get_modules((result) => {
      modules_list = result.message.pages;
      active_module = result.message.pages.filter((module) => {
        return module.name == _module;
      })[0];
      $(".splash").hide();
      if (active_module && active_module.name) {
...
const after_side_menu = () => {
  ...
  current_page = $('.content.page-container:visible', document);
};
...
const after_side_menu_items = () => {
  ...
  setTimeout(() => {
    ...
    $('.side-menu .side-menu-items > ul.dropdown-list > li, .side-menu ul.mobile-modules-menu-list > li', document).each(function () {
      ...
      $('.side-menu .side-menu-items > ul.dropdown-list > li.active, .side-menu ul.mobile-modules-menu-list > li.active').each(function () {
        if (!$('>ul>li>a[data-label="' + current_doctype + '"]', this).length) {
          $(this).removeClass('active hide-sub-menu').find('>ul').hide();
        }
      });

      $('>ul>li>a[data-label="' + current_doctype + '"]', li).each(function () {
        if (!$(this).parent().hasClass('active')) {
          $('>ul>li>a[data-label="' + current_doctype + '"]', li).parent().addClass('active');
        }
  ...
  setTimeout(() => {
    let current_dashboard_link = $('.side-menu ul.mobile-modules-menu-list > li > a[data-dashboard="' + current_doctype + '"]', document);
    current_dashboard_link.parent().addClass('active');
  }, 520);
};

const get_theme_settings = (callback) => {
  if (theme_settings && Object.keys(theme_settings).length) {
    callback();
  } else {
    frappe.call({
      type: 'POST',
      ...
      args: {},
      callback: function (response) {
        theme_settings = response.message;
        callback();
      }
    });
  }
...
const open_module = (module, is_mobile = false) => {
  ...
  if (is_mobile == false) {
    ...
    if (theme_settings.open_workspace_on_mobile_menu && theme_settings.open_workspace_on_mobile_menu == 1) {
      frappe.set_route('/' + (module.replace(/ /g, "-")).toLowerCase());
      is_shown_mobile_menu = false;
    } else {
      is_shown_mobile_menu = true;
    }
  }
};

const open_dashboard = (module) => {
  frappe.set_route('/dashboard-view/' + (module));
};

const mobile_back_modules = () => {
  is_shown_mobile_menu = false;
};

const get_module_icon = (name, icon) => {
...
onMounted(() => {
  ...
  if ($('body').hasClass('frappe-rtl')) {
    menu_items_animate = 'slideInRight';
  }
  $(document).on('page-change', () => {
    get_theme_settings(() => {
      get_module_name_from_doctype();
    });
  });
});
