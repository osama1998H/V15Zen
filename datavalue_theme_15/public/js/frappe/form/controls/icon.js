...
frappe.ui.form.ControlIcon = class ControlIcon extends frappe.ui.form.ControlData {
    ...
    refresh() {
        ...
        if (this.$input && this.$input.length) {
            let icon_element = this.$input.parent().parent().next('.static-area').find('.selected-icon');
            if (icon && icon.length && icon_element && icon_element.length)
                this.$input.parent().parent().next('.static-area').find('.selected-icon').html(`<i class="d-block mr-1 ${icon}"></i>`);
        }
